...
Výsledné zařízení dokáže měřit ve frekvenčním rozsahu do řádu jednotek\,\si{\GHz}, vzorkovací krok měření je 20\,\si{\ps}, ekvivalentní vzorkovací kmitočet je tedy 50\,\si{\gigasample}. Tento vzorkovací krok teoreticky umožňuje rozlišovací schopnost polohy závady na vedení 0,3\,\si{\cm} ve vakuu, v reálném prostředí může být i lepší. Samostatně dokáže zařízení detekovat jednoduché závady, jejich typ a polohu. V součinnosti s počítačem je možné provést i kalibraci pomocí kalibrační sady pro korekci nedokonalostí zařízení.
...
The resulting device is able to measure up to units of\,\si{\GHz}, sampling step is 20\,\si{\ps}, resulting in equivalent sampling rate of 50\,\si{\gigasample}. This sampling step theoretically allows spatial resolution of 0.3\,\si{\cm} in vacuum, possibly even less in real environment. The device can detect simple faults on its own, along with their type and position. When used with computer, it is possible to perform calibration using calibration set.
